...
export default async function PalutoTestPage() {
  ...
  const renderRaw = (section: any) => (
    <pre style={{ background: '#f4f4f4', padding: '10px', fontSize: '12px', overflow: 'auto', maxHeight: '200px' }}>
  ...
  return (
    <main style={{ padding: '40px', fontFamily: 'sans-serif', lineHeight: '1.6' }}>
      ...
      <section>
        <h2>2. About Us</h2>
        {data?.aboutUs ? renderRaw(data.aboutUs) : <p style={{color: 'red'}}>❌ No About Us data found</p>}
      </section>

      <section>
        <h2>3. Showcase Logo</h2>
        {data?.showcaseLogo ? renderRaw(data.showcaseLogo) : <p style={{color: 'red'}}>❌ No Showcase Logo data found</p>}
      </section>

      <section>
        <h2>4. Showcase</h2>
        {data?.showcase ? renderRaw(data.showcase) : <p style={{color: 'red'}}>❌ No Showcase data found</p>}
      </section>

      <section>
        <h2>5. Feedback</h2>
        {data?.feedback ? renderRaw(data.feedback) : <p style={{color: 'red'}}>❌ No Feedback data found</p>}
      </section>

      <section>
        <h2>6. Contact Us</h2>
        {data?.contactUs ? renderRaw(data.contactUs) : <p style={{color: 'red'}}>❌ No Contact Us data found</p>}
      </section>
