	...
	return (
		<main className="w-full pb-24">
			...
			<div className="container mx-auto px-4 mt-20 space-y-28">
				...
				{/* 3. VIDEO SECTION */}
				{data.overviewVideo?.href && (
					<CareersVideoPlayer url={data.overviewVideo.href} />
				)}
						...
						<div className="relative aspect-square rounded-2xl overflow-hidden shadow-lg">
							{getImgUrl(data.valuesSection.image) && (
								<Image
									...
									alt={getAltText(data.valuesSection.image, data.valuesSection.title || "Values Image")}
									fill
									className="object-cover"
								/>
							)}
							...
							{data.whyJoinUs.description?.map((block) => {
								if (block.type === "list") {
									return block.children?.map((listItem) => {
										const bulletText =
											listItem.children?.map((c) => c.text).join("") || "";
										return (
											<div
												key={bulletText}
												className="flex items-start text-xs text-gray-700"
											>
												<span className="mr-3 mt-1 text-blue-600 font-bold">
													•
												</span>
