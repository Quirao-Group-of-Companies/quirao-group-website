...
export default async function ManokanPage() {
  ...
  const aboutUsDescription = aboutUsData?.description || "Sari-sari Manokan and Seafood Restaurant is a seafood destination in Iloilo, known for fresh seafood and celebration-ready ambiance.";
  ...
  const showcaseData = pageData.Showcase?.map((item: StrapiCards) => ({
    id: item.id,
    label: item.title || '',
  ...
  const feedbackData = pageData.feedback?.map((item: StrapiFeedback) => ({
    id: item.id,
  ...
  return (
    <main className="w-full min-h-screen bg-qgc-white">
      ...
      <section className="bg-qgc-gray-soft px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-10 md:h-screen overflow-hidden py-16 md:py-10">
        ...
        <div className="w-full md:w-[40%] flex justify-center items-center h-100 md:h-[80%] lg:h-[85%] relative rounded-4xl overflow-hidden shadow-2xl">
          <Image
            src={aboutUsImage}
