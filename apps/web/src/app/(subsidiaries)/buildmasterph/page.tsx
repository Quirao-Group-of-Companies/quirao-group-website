...
export default async function BuildMasterPage() {
  ...
  return (
    <main className="w-full min-h-screen bg-white">
      ...
      {/* ── APP DOWNLOAD BANNER ── */}
      <section className="relative w-full overflow-hidden bg-bm-blue`">
        {bannerSrc ? (
          <Image
            src={bannerSrc}
          ...
          <div
            className="w-full bg-linear-to-r from-bg-bm-blue to-[#1a4a9a]"
        ...
        >
          <DownloadButton
            href={appStoreHref}
            variant="apple"
            label={cms.download?.cta?.[1]?.title ?? 'App Store'}
          />
          <DownloadButton
            href={googlePlayHref}
            variant="google"
            label={cms.download?.cta?.[0]?.title ?? 'Google Play'}
          />
        </div>
      </section>
